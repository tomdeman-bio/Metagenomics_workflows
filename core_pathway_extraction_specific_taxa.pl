...
GetOptions (
    "humann2=s" =>  \$humman2,
    "coreRatio=f" =>  \$ratio,
    "speciesFilter=s" =>  \$species_filter,
    "outFile=s" =>  \$outfile)
or die ("Error in command line arguments\n");

#parsing command line arguments
if (($humman2) && ($ratio) && ($outfile) && ($species_filter)) {
...
sub parsehumman2 {
    ...
    while (<HUMANN2>) {
        ...
        #Only focus on taxa specific pathway listings... for now - merge versions later
        ...
        my $pathway = $pathway_taxa_for_split[0];
